...
function activate(context) {
    ...
    const refinePromptCmd = vscode.commands.registerCommand("promptos.refinePrompt", async () => {
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            void vscode.window.showWarningMessage("PromptOS: No active editor found.");
            return;
        }
        ...
        if (!text.trim()) {
            void vscode.window.showWarningMessage("PromptOS: Please select some text or intent to refine.");
            return;
        }
        // Show "Compiling..." in status bar
        // 在状态栏显示正在编译，提升用户体验
        const statusBarMsg = vscode.window.setStatusBarMessage("PromptOS: Compiling intent via LLM...");
        try {
            // A. Load Configuration (Env or ~/.promptos/config.json)
            // 加载配置。如果未配置 Key，此处会抛出友好的错误。
            const config = (0, llm_core_1.loadConfig)();
            // B. Compile Intent -> .pos YAML
            // 调用 SDK 进行编译
            const compiledYaml = await (0, sdk_1.compileIntentToPos)(text, config);
            ...
            void vscode.window.showInformationMessage("PromptOS: Intent compiled successfully!");
            // D. Save to History (Background Task)
            // 后台保存历史记录到 .promptos/history，不阻塞主流程
            if (vscode.workspace.workspaceFolders &&
                vscode.workspace.workspaceFolders.length > 0) {
                const rootPath = vscode.workspace.workspaceFolders[0].uri.fsPath;
                (0, sdk_1.saveToHistory)(rootPath, {
                    intent: text,
            ...
            if (message.includes("Missing API Key")) {
                const selection = await vscode.window.showErrorMessage(`PromptOS: LLM Configuration missing.`, "Open Config File", "Cancel");
                // Future: Implement "Open Config File" logic to open ~/.promptos/config.json
            }
            else {
                void vscode.window.showErrorMessage(`PromptOS Compiler failed: ${message}`);
            }
        }
        finally {
