...
exports.resolveLLMConfigForVSCode = resolveLLMConfigForVSCode;
exports.handleGeneratePromptFromDsl = handleGeneratePromptFromDsl;
exports.handleRunDslFromCurrentLine = handleRunDslFromCurrentLine;
exports.handleExportAsSkill = handleExportAsSkill;
// apps/host-vscode/src/promptosService.ts
const vscode = __importStar(require("vscode"));
const fs = __importStar(require("fs"));
const path = __importStar(require("path"));
const sdk_1 = require("@promptos/sdk");
const llm_core_1 = require("@promptos/llm-core");
const abilities_1 = require("@promptos/abilities");
const sdk_2 = require("@promptos/sdk");
/**
 ...
 */
function normalizeProviderId(raw, fallback) {
    if (!raw)
        return fallback;
...
async function buildTaskNoteForVSCode(args) {
    ...
    const useClipboard = config.get("useClipboardAsFallbackContext", true);
    let clipboardText = "";
    if (useClipboard) {
 ...
 * 该配置函数主要为后续 LLM 策略预留。
 */
function resolveLLMConfigForVSCode() {
    // 1) start with env-based config
    // 1）从环境变量获取基础配置
    const baseConfig = (0, llm_core_1.configFromEnv)();
    ...
    const cfgProvider = vscodeConfig.get("llm.provider");
    const cfgApiKey = vscodeConfig.get("llm.apiKey");
    const cfgModel = vscodeConfig.get("llm.model");
    const cfgBaseUrl = vscodeConfig.get("llm.baseUrl");
    const merged = {
        ...baseConfig,
        provider: normalizeProviderId(cfgProvider || fileProvider, baseConfig.provider),
 ...
 * 确保当前有激活的编辑器，否则提示错误。
 */
function getActiveEditorOrShowError() {
 ...
 */
async function openPromptInMarkdownDocument(prompt) {
 ...
 */
async function handleGeneratePromptFromDsl(_context) {
    const editor = getActiveEditorOrShowError();
    if (!editor)
        return;
    // 1. Ability Picker Step
    // 创建 QuickPick 让用户搜索能力
    const items = abilities_1.builtinAbilities.map((a) => ({
    ...
    try {
        const result = await (0, sdk_1.generatePrompt)(sdkArgs);
        await openPromptInMarkdownDocument(result.prompt);
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        void vscode.window.showErrorMessage(`PromptOS Generation Failed: ${message}`);
    }
}
/**
 ...
 */
async function handleRunDslFromCurrentLine(_context) {
    const editor = getActiveEditorOrShowError();
    if (!editor)
        return;
    ...
    if (!trimmed.startsWith("op:")) {
        void vscode.window.showErrorMessage("Current line is not a PromptOS command (missing 'op:' prefix).");
        return;
    }
    // 提取 DSL 部分
    ...
    try {
        const result = await (0, sdk_1.generatePrompt)(args);
        await openPromptInMarkdownDocument(result.prompt);
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        void vscode.window.showErrorMessage(`PromptOS Execution Failed: ${message}`);
    }
}
/**
 ...
 */
async function handleExportAsSkill(_context) {
    ...
    try {
        ...
        const workspaceFolders = vscode.workspace.workspaceFolders;
        if (!workspaceFolders || workspaceFolders.length === 0) {
            void vscode.window.showErrorMessage("No workspace open. 请先打开一个项目文件夹。");
            return;
        }
        ...
        const skill = await (0, sdk_2.exportToSkillStandard)(ability, ability.staticPromptTemplate || "");
        ...
        // 7. 成功反馈
        const openDoc = "立即查看";
        const choice = await vscode.window.showInformationMessage(`✅ 已导出至 ${skill.directory}`, openDoc);
        if (choice === openDoc) {
...
//# sourceMappingURL=promptosService.js.map