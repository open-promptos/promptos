// packages/prompt-writer/src/LLMPromptWriter.ts
import type { LLMClient, LLMMessage, LLMChatOptions } from "@promptos/llm-core";
import type {
  AbilityMeta,
  PromptWriterInput,
  PromptWriterSystemTemplate,
} from "./types";
 ...
 */
export class LLMPromptWriter {
  private readonly client: LLMClient;
  private readonly defaultSystemTemplate?: PromptWriterSystemTemplate;
  ...
  constructor(
    client: LLMClient,
    defaultSystemTemplate?: PromptWriterSystemTemplate,
  ) {
    this.client = client;
  ...
  async write(
    ...
    options?: PromptWriterWriteOptions,
  ): Promise<string> {
  ...
  protected buildSystemMessage(
    ...
    const fromTemplate =
      options?.systemTemplate?.systemPrompt ??
  ...
  protected extractCodeLanguage(taskNote?: string): string | undefined {
    if (!taskNote) return undefined;

    const match = taskNote.match(
      /\[Code Language\]\s*([\s\S]*?)(?:\n\[[^\]]+\]|\n*$)/i,
    );
  ...
  protected buildUserMessage(input: PromptWriterInput): LLMMessage {
    ...
    lines.push(
      "Your job is to craft the best possible single prompt for a downstream LLM, based on the information below.",
      "你需要根据以下信息，为下游大模型生成一条高质量的单条 Prompt。", // Chinese clarification
      "",
    );
    ...
    lines.push("");
    lines.push(
      "Now, based on everything above, write a single final prompt for the downstream LLM.",
    );
    lines.push(
      "Only output the prompt text itself, without explanations, comments, or markdown fences.",
    );
    lines.push(
      "现在请基于以上信息，为下游大模型生成一条完整的 Prompt，只输出 Prompt 本身，不要解释或添加其他内容。",
    );
